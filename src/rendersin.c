...
*/
void RenderSin(UInt32 startingFrameCount, UInt32 inFrames, 
		...
		int inOutputFormat)
{
	...
	if (startingFrameCount >= sampleNextPrinted) {
		printf("Current Slice: inFrames=%u, startingFrameCount=%u\n",
			(unsigned int)inFrames, (unsigned int)startingFrameCount);
